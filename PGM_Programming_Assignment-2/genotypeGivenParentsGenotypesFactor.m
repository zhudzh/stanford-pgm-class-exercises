...
card = numAlleles + factorial(numAlleles)/(2*factorial(numAlleles-2));
...
for i=1:card,
    for j=1:card,
        p1Alleles = genotypesToAlleles(i,:);
        p2Alleles = genotypesToAlleles(j,:);
        for a=1:2,
            for b=1:2,
                c = allelesToGenotypes(p1Alleles(a), p2Alleles(b));
                v = GetValueOfAssignment(genotypeFactor, [c,i,j]);
                genotypeFactor = SetValueOfAssignment(genotypeFactor, ...
                                                      [c,i,j], v + 0.25);
            end
        end
        % c11 = allelesToGenotypes(p1Alleles(1), p2Alleles(1));
        % c12 = allelesToGenotypes(p1Alleles(1), p2Alleles(2));
        % c21 = allelesToGenotypes(p1Alleles(2), p2Alleles(1));
        % c22 = allelesToGenotypes(p1Alleles(2), p2Alleles(2));
        
        % v = GetValueOfAssignment(genotypeFactor, [c11,i,j]);
        % genotypeFactor = SetValueOfAssignment(genotypeFactor, [c11,i,j], v + 0.25);
        
        % v = GetValueOfAssignment(genotypeFactor, [c12,i,j]);
        % genotypeFactor = SetValueOfAssignment(genotypeFactor, [c12,i,j], v + 0.25);

        % v = GetValueOfAssignment(genotypeFactor, [c21,i,j]);
        % genotypeFactor = SetValueOfAssignment(genotypeFactor, [c21,i,j], v + 0.25);

        % v = GetValueOfAssignment(genotypeFactor, [c22,i,j]);
        % genotypeFactor = SetValueOfAssignment(genotypeFactor, [c22,i,j], v + 0.25);
    end
end

...
gene_card = zeros(1, length(alleleWeights));
for i=1:length(alleleWeights),
    numAllele = length(alleleWeights{i});
    gene_card(i) = numAllele;
end
phenotypeFactor.card = [2, gene_card, gene_card];

phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.
for i = 1:phenotypeFactor.card(2),
    for j = 1:phenotypeFactor.card(3),
        for k = 1:phenotypeFactor.card(4),
            for l = 1:phenotypeFactor.card(5),
                z = alleleWeights{1}(i) + alleleWeights{2}(j) ...
                    + alleleWeights{1}(k) + ...
                    alleleWeights{2}(l);
                s = computeSigmoid(z);
                phenotypeFactor.val(AssignmentToIndex([1,i,j,k,l], ...
                                                      phenotypeFactor.card)) = s;
                phenotypeFactor.val(AssignmentToIndex([2,i,j,k,l], ...
                                                      phenotypeFactor.card)) = 1-s;
            end
        end
    end
end

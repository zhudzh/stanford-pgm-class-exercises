...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
for geno_index = 1:numPeople,
    if (isfield(pedigree, 'parents')),
        pa = pedigree.parents(geno_index,:);
    else
        pa = [0,0];
    end
    if (pa == [0,0]),
        factorList(geno_index) = ...
            childCopyGivenFreqsFactor(alleleFreqs, ...
                                      geno_index);
        factorList(geno_index + numPeople) = ...
            childCopyGivenFreqsFactor(alleleFreqs, ...
                                      geno_index + numPeople);
    else
        factorList(geno_index) = ...
            childCopyGivenParentalsFactor(numAlleles, geno_index, ...
                                          pa(1), pa(1)+numPeople);
        factorList(geno_index+numPeople) = ...
            childCopyGivenParentalsFactor(numAlleles, geno_index+numPeople, ...
                                          pa(2), pa(2)+numPeople);
    end
end

for pheno_index = 2*numPeople+1:numPeople*3,
    factorList(pheno_index) = ...
        phenotypeGivenCopiesFactor(alphaList, numAlleles, pheno_index ...
                                   - (2*numPeople), pheno_index - numPeople,
                                     pheno_index);
end

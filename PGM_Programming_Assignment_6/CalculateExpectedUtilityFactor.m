...
function EUF = CalculateExpectedUtilityFactor( I )
  ...
  for i = 1:length(I.RandomFactors),
      vs = [vs I.RandomFactors(i).var];
  end
  for i = 1:length(I.DecisionFactors),
      dvs = [dvs I.DecisionFactors(i).var];
  end
  vs = unique(vs);
  dvs = unique(dvs);
  FP = struct('var', [], 'card', [], 'val', []);
  Fnew = VariableElimination([I.RandomFactors U], setdiff(vs, dvs));
  for j = 1:length(Fnew),
      FP = FactorProduct(FP, Fnew(j));
  end
  EUF = FP;
end  

...
M = struct('var', [], 'card', [], 'val', []); % Returns empty
                                              % factor. Change
                                              % this.
evidence = ObserveEvidence(F, E);
% for i=1:length(evidence),
%     s = sum(evidence(i).val);
%     evidence(i).val = evidence(i).val / s;
% end

joint = ComputeJointDistribution(evidence);

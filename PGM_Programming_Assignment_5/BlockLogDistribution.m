...
if (length(V) == 1),
    ...
    factorIndex = [];
    for i = 1:length(V),
        factorIndex = union(factorIndex, G.var2factors{V(i)});
    end
end
result = zeros(length(factorIndex), 2);
for i = 1:length(factorIndex),
    f = F(factorIndex(i));
    E = [f.var', A(f.var)'];
    E(lookup(f.var, intersect(V, f.var)), 2) = 1;
    result(i, 1) = log(GetValueOfAssignment(f, E(:, 2)'));
    E(lookup(f.var, intersect(V, f.var)), 2) = 2;
    result(i, 2) = log(GetValueOfAssignment(f, E(:, 2)'));
end

LogBS = sum(result);

% for i = 1:length(factorIndex),
%     joint = FactorProduct(joint, F(factorIndex(i)));
% end

% E = [joint.var', A(joint.var)'];
% for val = 1:d,
%     Es = E;
%     Es(lookup(joint.var, V), 2) = val;
%     LogBS(1, val) = log(GetValueOfAssignment(joint, Es(:, 2)'));
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
